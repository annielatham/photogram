class LikesController < ApplicationController
  ...
  def current_user_must_be_like_user
    ...
    unless current_user == like.liking_user
  ...
  def index
    @q = current_user.likes.ransack(params[:q])
      @likes = @q.result(:distinct => true).includes(:liking_user, :photo).page(params[:page]).per(10)
  ...
  def create
    ...
    @like.liking_user_id = params[:liking_user_id]
