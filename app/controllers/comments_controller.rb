class CommentsController < ApplicationController
  ...
  def current_user_must_be_comment_user
    ...
    unless current_user == comment.commenting_user
  ...
  def index
    @q = current_user.comments.ransack(params[:q])
      @comments = @q.result(:distinct => true).includes(:commenting_user, :photo).page(params[:page]).per(10)
  ...
  def create
    ...
    @comment.commenting_user_id = params[:commenting_user_id]
